...
/*   Updated: 2017/05/23 15:05:10 by gthomas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (inst[0] == DIRECT_CHAR && inst[1] != LABEL_CHAR)
	{
		...
		else
		{
			if (!ft_nisdigit(inst + 1, ft_strlen(inst + 1)))
			{
				...
				if (inst[1] == '-' && !ft_nisdigit(inst + 2, ft_strlen(inst + 2)))
					return (0);
				if (ft_atoi(inst + 1) > IDX_MOD || ft_atoi(inst + 1) < -IDX_MOD)
					return (0);
			}
		}
	}
...
int		check_ind(t_asm *vasm, char *inst)
{
	if (vasm->command >= 13 && vasm->command <= 15)
	{
		if (!ft_nisdigit(inst, ft_strlen(inst)) && (inst[0] != '-' || (inst[0] == '-' && !ft_nisdigit(inst + 1, ft_strlen(inst + 1)))))
				return (0);
	}
	else
	{
